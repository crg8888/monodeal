-- Postgres 17 stricter about scalar-subquery-returning-array in ANY().
-- Replace `v_id = any((select bank from players ...))` with EXISTS subqueries.
...
begin
  ...
  foreach v_id in array p_card_ids loop
    select exists(select 1 from players where id = p_actor_id and v_id = any(bank))
      into v_in_bank;
    if v_in_bank then
